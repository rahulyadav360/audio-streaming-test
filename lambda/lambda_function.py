...
class PlayNewestEpisodeIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        persistent_attributes["playback_session_data"] = { 'token': token, 'url': url,'offset': offset, 'title': title, 'index': index, 'loop': False, 'shuffle': False }
        handler_input.attributes_manager.save_persistent_attributes()
...
class ChooseEpisodeIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        subtitle = "Episode {}".format(token)
        persistent_attributes['playlist'] = playlist
        persistent_attributes["playback_session_data"] = { 'next_track_queued':False, 'token': token, 'url': url, 'offset': offset, 'title': title, 'index': index, 'loop': False, 'shuffle': False }
        handler_input.attributes_manager.save_persistent_attributes()
...
class NextIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        title = playlist[index]['title']
        subtitle = "Episode {}".format(token)
        persistent_attributes["playback_session_data"].update({ 'token': token, 'url': playlist[index]['url'], 'offset': offset, 'title': playlist[index]['title'], 'index': index})
        handler_input.attributes_manager.save_persistent_attributes()        
...
class ShuffleOnIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        persistent_attributes["playback_session_data"].update({"index": index, "shuffle": True})
        handler_input.attributes_manager.save_persistent_attributes()
...
class PlaybackStoppedEventHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        #index = int(audio_player_attributes.token)
        index = int(persistent_attributes["playback_session_data"]["index"])
        token = str(index+1)
        playlist = persistent_attributes['playlist']
...
class PlaybackNearlyFinishedEventHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        persistent_attributes["playback_session_data"].update({ 'next_track_queued': True })
        handler_input.attributes_manager.save_persistent_attributes()
...
class PlaybackFinishedEventHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        persistent_attributes["playback_session_data"].update({ 'next_track_queued': False })
        handler_input.attributes_manager.save_persistent_attributes()
        return handler_input.response_builder.response
